...
// When both the CTE and orientation errors are zero, we still need to ensure the vehicle is moving
// We can use either euclidean distance or reference velocity constraint
double ref_v = 80;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
      ...
      // Then for actuator constraints
      // i.e magnitude constraints
      for(int t = 0; t < N-1; t++){
          fg[0] += 1000*CppAD::pow(vars[delta_start + t], 2);
          fg[0] += 10*CppAD::pow(vars[a_start + t], 2);
      }
      
      // Minimize the gap between successive actuations
      for(int t = 0; t < N-2; t++){
      ...
      for (int t = 1; t < N; t++) {
          ...
          AD<double> f0 = coeffs[0] + coeffs[1]*x0 + coeffs[2]*CppAD::pow(x0, 2) + coeffs[3]*CppAD::pow(x0, 3); //Order 3 polynomial
          AD<double> psides0 = CppAD::atan(coeffs[1] + 2*coeffs[2]*x0 + 3*coeffs[3]*CppAD::pow(x0, 2));
          
          // Dynamic Constraints
          fg[1 + x_start + t] = x1 - (x0 + v0 * CppAD::cos(psi0) * dt);
          fg[1+ y_start + t] = y1 - (y0 + v0 * CppAD::sin(psi0) * dt);
          fg[1 + psi_start + t] = psi1 - (psi0 + v1 * delta0 * dt / Lf);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // creates a 2 element double vector.
    ...
    for(int i = 0; i < N-1; ++i){
        //cout << "mpc values = " << solution.x[x_start + i] << endl;
